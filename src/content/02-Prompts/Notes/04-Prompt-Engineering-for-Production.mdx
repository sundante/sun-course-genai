...
```mermaid
flowchart LR
    A["✏️ Draft"] --> B["🧪 Test<br/>(eval set)"] --> C["👀 Review"] --> D["🚀 Deploy"] --> E["📊 Monitor"] --> F["🔁 Iterate"]
    F -.-> A
```
...
Defense in depth: no single layer is sufficient, but overlapping layers reduce the attack surface.

```mermaid
flowchart TD
    IN["📥 Untrusted input<br/>(user text, retrieved documents)"] --> D1["🧱 Defense 1: Structural delimiters<br/>Wrap untrusted content in XML tags,<br/>instruct model not to execute it"]
    D1 --> D2["🧹 Defense 2: Input sanitization<br/>Pattern-match and block known<br/>injection phrases before they reach the model"]
    D2 --> LLM["🤖 LLM call"]
    LLM --> D3["🔍 Defense 3: Output monitoring<br/>Flag responses that leak system<br/>prompt contents or break character"]
    D3 --> OUT(["✅ Response to user"])

    style D1 fill:#e8f4fd,stroke:#4a9eca
    style D2 fill:#fff3cd,stroke:#f0a500
...
```mermaid
xychart-beta
    title "Attention distribution across a long context window"
    x-axis "Position" ["beginning", "early", "middle", "late", "end"]
