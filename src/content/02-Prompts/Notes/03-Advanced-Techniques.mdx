...
**Standard CoT** - one path:
```mermaid
flowchart LR
    P["Problem"] --> S1["Step 1"] --> S2["Step 2"] --> S3["Step 3"] --> A(["Answer"])
```

**Tree of Thought** - explore, evaluate, backtrack:
```mermaid
flowchart LR
    P["Problem"] --> R["Root"]
    R --> A1["Branch A1"] --> A2["A2"] --> AX(["❌ dead end, backtrack"])
    R --> B1["Branch B1"] --> B2["B2"] --> B3["B3"] --> BX(["✅ Answer"])
    R --> C1["Branch C1"] --> CX(["🗑️ low quality, pruned"])

    style BX fill:#d4edda,stroke:#28a745
    style AX fill:#f8d7da,stroke:#dc3545
    style CX fill:#f8d7da,stroke:#dc3545
```
...
```mermaid
flowchart LR
    P["📝 Prompt<br/>(same each time, temperature &gt; 0)"] --> C1["Chain 1 → 42"]
    P --> C2["Chain 2 → 42"]
    P --> C3["Chain 3 → 40 ⚠️ outlier"]
    P --> C4["Chain 4 → 42"]
    P --> C5["Chain 5 → 42"]
    C1 & C2 & C3 & C4 & C5 --> V["🗳️ Majority vote"] --> R(["✅ 42"])
```
...
```mermaid
flowchart LR
    A["📝 Prompt 1<br/>Extract key claims"] -->|list of claims| B["✅ Prompt 2<br/>Verify each claim against source"]
    B -->|claim + verified/unverified| C["📄 Prompt 3<br/>Write balanced summary<br/>using only verified claims"]
    C --> D(["Final output"])
```
...
```mermaid
flowchart TD
    A["🧩 Stage 1 - Decompose<br/>'To solve [hard problem], I first need to<br/>solve: [sub-problem 1], then [sub-problem 2]...'"]
    A --> B["1️⃣ Solve sub-problem 1"] --> C["➕ Add solution to context"]
    C --> D["2️⃣ Solve sub-problem 2<br/>using sub-problem 1's answer as context"]
    D --> E["..."]
    E --> F["✅ Solve original problem<br/>using all sub-problem answers as context"]
```
