...
```mermaid
flowchart TD
    subgraph INGEST["📥 Ingest Pipeline (offline / async)"]
        DS["🗄️ Data Sources<br/>S3, GCS, SharePoint, Confluence, DB"] --> DP["⚙️ Document Processor<br/>Loader → Extract → Clean → Chunk → Embed"]
        DP <-.-> Q["📬 Queue (Pub/Sub, SQS)<br/>async processing"]
        DP --> VS[("🗄️ Vector Store<br/>upsert vectors + metadata")]
    end

    subgraph QUERY["🔍 Query Pipeline (online / synchronous)"]
        UQ["❓ User query"] --> QP["✏️ Query Preprocessing<br/>rewrite / expand / classify"]
        QP --> CACHE["⚡ Cache Layer<br/>semantic cache (Redis + vector search)"]
        CACHE -->|cache miss| RET["📡 Retriever<br/>hybrid (dense + BM25) → top-20"]
        RET --> RR["🎯 Reranker<br/>cross-encoder → top-4 chunks"]
        RR --> CA["🧩 Context Assembly<br/>format chunks + citations"]
        CA --> LLM["🤖 LLM Generation<br/>Gemini / GPT-4 with grounding prompt"]
        LLM --> PP["🧹 Post-processing<br/>citation formatting, PII scrubbing"]
        PP --> ANS(["✅ Answer + Source Citations"])
    end

    subgraph FEEDBACK["🔁 Feedback Loop (async)"]
        UF["👍 User Feedback"] --> ES["🗂️ Eval Store"] --> QM["📊 Quality Metrics"] --> TR["🔧 Trigger Re-index /<br/>Rerank Tuning"]
    end
    ...
    style QUERY fill:#d4edda,stroke:#28a745
    style FEEDBACK fill:#fde8d8,stroke:#fd7e14
```
...
```mermaid
flowchart TD
    subgraph ARCH["🏗️ Architecture for 100M documents"]
        I["☁️ Ingest Service (Cloud Run)<br/>Doc processor × 10 workers (Pub/Sub queue)<br/>Batch embedding (text-embedding-004, batch=512)"]
        V["📡 Vertex AI Vector Search (managed HNSW)<br/>Index: 120M vectors, 768-dim, cosine<br/>Read replicas: 3 (auto-scaled by QPS)<br/>Deployed endpoint: HTTP call → top-k"]
        Q["☁️ Query Service (Cloud Run, min-instances=3)<br/>Semantic cache (Cloud Memorystore Redis)<br/>Hybrid search (Vector Search + Elasticsearch BM25)<br/>Reranker (MiniLM on Cloud Run GPU)<br/>Gemini API for generation"]
        I --> V --> Q
    end

    style ARCH fill:#f4f1ec,stroke:#ccc4b8
```
...
```mermaid
flowchart TD
    A["🗄️ Vector DB unavailable"] --> A1["↪️ Fallback to BM25-only<br/>(Elasticsearch / in-memory index)"]
    A1 --> A2["↪️ If BM25 also unavailable → LLM with<br/>no context + uncertainty disclaimer"]

    B["🧬 Embedding model API unavailable"] --> B1["↪️ If query embedding fails →<br/>serve stale cache if available"]
    B1 --> B2["↪️ If no cache → fallback to BM25"]

    C["🤖 LLM API rate limited / unavailable"] --> C1["↪️ Queue request,<br/>return 'processing' response"]
    C1 --> C2["↪️ For sync requirement →<br/>switch to smaller/faster fallback model"]

    D["🎯 Reranker unavailable"] --> D1["↪️ Skip reranking,<br/>use first-stage retrieval order"]
    D1 --> D2["↪️ Monitor precision drop<br/>in downstream metrics"]
...
```mermaid
flowchart TD
    subgraph ING["📥 Ingest (daily batch, 9 PM)"]
        S["📄 SharePoint / Confluence"] --> DP["⚙️ Doc processor (Cloud Run)"]
        DP --> TC["✂️ Text clean + chunk (500 chars)"]
        TC --> BE["🧬 Batch embed (text-embedding-004)"]
        BE --> VS[("🗄️ Vertex AI Vector Search<br/>upsert, namespace=department")]
    end

    subgraph QRY["🔍 Query (real-time)"]
        EQ["🧑 Employee query"] --> AUTH["🔐 Auth check<br/>IAM → map user to department"]
        AUTH --> SC["⚡ Semantic cache (Redis)<br/>30-60% hit rate expected"]
        SC -->|cache miss| HR["📡 Hybrid retrieval<br/>Vector Search + Elasticsearch BM25<br/>filter={department: user_dept}"]
        HR --> RR["🎯 Cohere Rerank API<br/>top-20 → top-5"]
        RR --> GEN["🤖 Gemini Pro<br/>(generation, streaming)"]
        GEN --> ANS(["✅ Answer + citations<br/>doc title, department, URL"])
    end

    VS -.-> HR

    style ING fill:#e8f4fd,stroke:#4a9eca
    style QRY fill:#d4edda,stroke:#28a745
```
