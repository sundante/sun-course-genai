...
```mermaid
flowchart TD
    Q["❓ Query:<br/>What causes transformer attention<br/>to fail on long sequences?"] --> LLM["🤖 LLM generates hypothesis"]
    LLM --> H["📝 Hypothesis: Transformer attention fails<br/>due to the quadratic complexity O(n²)<br/>of self-attention..."]
    H --> EMB["🧬 Embed hypothesis<br/>(not query)"]
    EMB --> ANN["📡 ANN search using<br/>hypothesis vector"]
    ANN --> R(["✅ Better recall on<br/>technical documents"])
```
...
```mermaid
flowchart LR
    O["🎯 Original:<br/>authentication performance"] --> R["📚 Retrieve for all 4,<br/>deduplicate, rerank merged set"]
    V1["🔀 Variant 1:<br/>login speed and latency"] --> R
    V2["🔀 Variant 2:<br/>identity verification throughput"] --> R
    V3["🔀 Variant 3:<br/>OAuth token validation benchmarks"] --> R
```
...
```mermaid
flowchart LR
    S["🎯 Specific:<br/>What is the melting point<br/>of sodium chloride?"] --> B["🔭 Step-back:<br/>What are the physical properties<br/>of ionic compounds?"]
    B --> R(["✅ Retrieve broader context first,<br/>then narrow the answer"])
```
