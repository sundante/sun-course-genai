...
flowchart TD
    subgraph IDX["📥 Index Phase (offline)"]
        D["📄 Documents"] --> CH["✂️ Chunk"] --> EMB["🧬 Embed"] --> VDB[("🗄️ Vector DB")]
    end
    subgraph QRY["🔍 Query Phase (online)"]
        Q["❓ User query"] --> EQ["🧬 Embed query<br/>(same model as indexing)"]
        EQ --> ANN["📡 ANN search"] --> TOPK["📚 Top-k chunks"]
        TOPK --> PR["📝 Build prompt<br/>context + query"]
        PR --> LLM["🤖 LLM generates<br/>grounded answer"]
    end
    VDB -.-> ANN
...
```mermaid
flowchart LR
    A["📄 Raw Documents"] --> B["📥 Document loader<br/>(PDF, HTML, DOCX, etc.)"]
    B --> D["✂️ Text splitter<br/>(chunking strategy)"]
    D --> F["🧬 Embedding model<br/>(e.g., text-embedding-004)"]
    F --> H[("🗄️ Vector Database<br/>Chroma, Pinecone, Vertex AI")]
```

**Query Phase (online, per-request)**
```mermaid
flowchart LR
    A["❓ User Query"] --> B["🧬 Embedding model<br/>(same model as index phase!)"]
    B --> D["📡 ANN search<br/>(k=4-8 typical)"]
    D --> E["📚 Top-k Chunks + Metadata"]
    E --> F["📝 Prompt template"]
    F --> G["🧾 Augmented Prompt"]
    G --> H["🤖 LLM<br/>(Gemini, GPT-4, Claude)"]
    H --> I(["✅ Answer"])
```
