...
```mermaid
flowchart LR
    A["🟢 Naive RAG"] -->|"add pre/post-retrieval steps"| B["🟡 Advanced RAG"]
    B -->|"decompose into swappable modules"| C["🟠 Modular RAG"]
    C -->|"add autonomous retrieval decisions"| D["🔴 Agentic RAG"]
    D -->|"replace vector index with knowledge graph"| E["🟣 GraphRAG"]
    E -->|"retrieve across image, text, audio"| F["🔵 Multimodal RAG"]
...
flowchart LR
    subgraph IDX["📥 Index"]
        D["📄 Documents"] --> CH["✂️ Chunk"] --> EM["🧬 Embed"] --> VDB[("🗄️ Vector DB")]
    end
    subgraph QRY["🔍 Query"]
        Q["❓ Query"] --> EQ["🧬 Embed"] --> SS["📡 Similarity search"] --> TK["📚 Top-k chunks"] --> PR["📝 Prompt"] --> LLM["🤖 LLM"] --> A(["✅ Answer"])
    end
    VDB -.-> SS

    style IDX fill:#e8f4fd,stroke:#4a9eca
...
flowchart LR
    ...
    R -->|"factual / internal"| A["🗄️ Vector DB retriever"] --> A2["context"] --> LLM1["🤖 LLM"]
    R -->|"recent events"| B["🌐 Web search"] --> B2["snippet"] --> LLM2["🤖 LLM"]
    R -->|"structured data"| C["🗃️ SQL agent"] --> C2["table"] --> LLM3["🤖 LLM"]
    R -->|"relationship"| D["🕸️ Graph traversal"] --> D2["paths"] --> LLM4["🤖 LLM"]
...
```mermaid
flowchart TD
    A["📚 Document corpus"] --> B["🤖 LLM extracts entities<br/>and relationships from each chunk"]
    B --> C["🔗 Entity-Relationship triples<br/>(Entity_A, relation, Entity_B)"]
    C --> D["🕸️ Build knowledge graph<br/>(nodes=entities, edges=relations)"]
    D --> E["🧩 Run community detection<br/>(Leiden algorithm)"]
    E --> F["👥 Communities<br/>(groups of related entities)"]
    F --> G["🤖 LLM generates summary<br/>for each community, multiple levels"]
    G --> H["📊 Hierarchical community summaries<br/>(C0 = coarse, C4 = fine-grained)"]
```
