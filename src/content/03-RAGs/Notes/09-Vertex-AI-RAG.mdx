...
```mermaid
flowchart TD
    Q["❓ User query"] --> CR["☁️ Cloud Run (query service)"]
    CR --> D["🧬 text-embedding-004 →<br/>Vertex AI Vector Search (dense k=20)"]
    CR --> E["🔤 Elasticsearch on GKE<br/>(BM25 k=20)"]
    D --> F["🔀 RRF fusion → 20 candidates"]
    E --> F
    F --> RR["🎯 Vertex AI ReRanker API<br/>→ top 5 chunks"]
    RR --> G["🤖 Gemini 1.5 Pro (generator)"]
    G --> R(["✅ Response + citations"])
```
...
```mermaid
flowchart LR
    subgraph A["🔍 Vertex AI Search<br/>'I need search'"]
        A1["Managed search over your data"]
        A2["Structured + unstructured"]
        A3["Not directly tied to LLM"]
    end
    subgraph B["⚡ Vertex AI RAG Engine<br/>'I need RAG with Gemini'"]
        B1["Managed retrieval corpus"]
        B2["Gemini integration, no infra"]
        B3["Only works with Gemini"]
    end
    subgraph C["🛠️ Custom RAG on GCP<br/>'I need full control'"]
        C1["AlloyDB/pgvector + Cloud Run,<br/>or Vector Search + own LLM"]
        C2["Full control of all stages"]
        C3["Bring any embedding model"]
    end
    A -.->|"more managed / less control"| B -.->|"more control / less managed"| C
...
```mermaid
flowchart TD
    G["🗄️ GCS bucket (raw docs)"] --> CI["☁️ Cloud Run ingest service"]
    CI --> E["🧬 text-embedding-004 (Vertex AI API)"]
    E --> V[("📡 Vertex AI Vector Search index")]
    V --> CQ["☁️ Cloud Run query service"]
    CQ --> H["🔀 Hybrid: Vector Search +<br/>Elasticsearch on GKE"]
    H --> RR["🎯 Cohere Rerank API"]
    RR --> LLM["🤖 Gemini API"]
```
...
```mermaid
flowchart TD
    A[("🐘 AlloyDB<br/>PostgreSQL + pgvector extension")]
    A --> B["📄 Store documents + metadata<br/>(SQL tables)"]
    A --> C["🔢 Store vectors<br/>(pgvector column)"]
    A --> D["🔀 Hybrid queries:<br/>SQL WHERE + &lt;=&gt; (cosine distance)"]
```
