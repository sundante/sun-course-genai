...
flowchart TD
    subgraph INGEST["📥 Ingestion Pipeline"]
        CS["🗄️ Cloud Storage<br/>(raw docs)"] --> DP["⚙️ Cloud Run: Document Parser<br/>PDF → Document AI (OCR + layout)<br/>Word/HTML → python-docx / BeautifulSoup<br/>Output: cleaned text + metadata JSON"]
        DP --> CT["📬 Cloud Tasks Queue<br/>(back-pressure management)"]
        CT --> CE["⚙️ Cloud Run: Chunker + Embedder<br/>Semantic chunking (512 tokens, 10% overlap)<br/>text-embedding-005 → 768-dim vectors"]
        CE --> VS[("📡 Vertex AI Vector Search<br/>(ANN)")]
        CE --> AD[("🗄️ AlloyDB<br/>(BM25 sparse index)")]
        CE --> BQ[("📊 BigQuery<br/>(metadata + eval logging)")]
    end

    subgraph QUERY["🔍 Query Pipeline"]
        UQ["❓ User Query"] --> GW["🚪 API Gateway → Query Service<br/>1. Auth check (IAP or OAuth2)<br/>2. Query embedding<br/>3. Check Memorystore semantic cache"]
        GW -->|cache miss| HR["📡 Hybrid Retrieval<br/>Dense: Vector Search → top-20<br/>Sparse: AlloyDB BM25 → top-20<br/>Fusion: RRF → top-5<br/>Rerank: Vertex AI Reranker → top-3"]
        HR --> GEN["🤖 Generation<br/>Prompt: system + context + query<br/>Gemini 1.5 Pro → answer + citations<br/>Grounding check + citation extraction"]
        GEN --> ANS(["✅ Response → User<br/>+ log to BigQuery"])
    end
