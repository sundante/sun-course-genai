...
├── Yes → Hobby agent. Keep it simple.
└── No → Who are the users?
    ├── Internal (employees only) → Internal enterprise agent
    │   Needs: auth, logging, cost controls
    └── External (customers, patients, public) → Production enterprise agent
        Needs: all of the above + compliance, PII handling, SLAs, legal review
```
