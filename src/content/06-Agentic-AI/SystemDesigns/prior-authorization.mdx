...
See the [End-to-End Agent Orchestration Flow](#end-to-end-agent-orchestration-flow) diagram in the Diagrams section below for the full architecture - submission surface, security & compliance layer, orchestrator, the three parallel sub-agents, submission readiness gap check, decision support, clinical reviewer queue, and notification agent.
...
```mermaid
flowchart TD
    A["📥 Provider submits PA request<br/>(EHR auto-trigger / portal / fax)"] --> B["🔒 Security Layer<br/>PHI de-id · prompt injection guard · audit log init"]
    B --> C["🧭 Orchestrator<br/>parse service code · set urgency tier"]
    C --> D["📋 Evidence Aggregator (parallel)"]
    C --> E["📚 Policy Interpreter (parallel)"]
    D --> F["🔌 Legacy System Connector<br/>prior PA history + clinical context"]
    E --> F
    F --> G["✅ Submission Readiness Agent"]
    G -->|COMPLETE| H["🧠 Decision Support Agent<br/>evidence → criteria mapping → confidence score"]
    G -->|INCOMPLETE| I["📨 Notify Provider of gaps"]
    I -->|provider responds| G
    H --> J["👩‍⚕️ Clinical Reviewer Queue<br/>SLA by urgency tier"]
    J --> K["✅ APPROVED"]
    J --> L["❌ DENIED"]
    J --> M["🔍 PEER-TO-PEER"]
    K --> N["🔔 Notification Agent<br/>EHR in-basket · letter · payer portal update<br/>Audit log finalised"]
    L --> N
    M --> N
```
...
See the [PA State Machine](#pa-state-machine-1) diagram in the Diagrams section below for the full state transition graph (SUBMITTED → EVIDENCE_GATHERING → ... → CLOSED).
