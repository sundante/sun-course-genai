...
```mermaid
flowchart TD
    IN["🧑‍🔧 Technician Input<br/>(voice / QR scan / photo)"] --> S1

    subgraph S1["1️⃣ Data Processing Agent"]
        V["🗣️ Voice → Vertex AI Speech-to-Text → problem description"]
        P["📷 Photo → Gemini Vision API → failure symptoms"]
        Q["🔎 QR Code → Equipment Registry lookup → machine context"]
        OUT1["📦 EquipmentContext {equipment_id, machine_model,<br/>location, failure_description, symptom_tags[],<br/>technician_id, skill_tier}"]
        V --> OUT1
        P --> OUT1
        Q --> OUT1
    end
    ...
    subgraph S2["2️⃣ Orchestrator"]
        O1["Classify failure mode (Gemini Flash) + confidence"]
        O2["Authorization check: skill_tier sufficient?<br/>If no → escalate immediately, skip all agents"]
        O3["Check LOTO requirement for this failure class"]
        O4["Decide which agents to invoke:<br/>SOPs & Safety: ALWAYS · RAG Manual: ALWAYS<br/>Legacy & History: only if repair history exists"]
        O1 --> O2 --> O3 --> O4
    end
    ...
    S3A["🕸️ 3A: Legacy & History Fetch<br/>Spanner Graph: machine → failure → procedure<br/>BigQuery: past repairs, MTTR, tech notes<br/>Returns: similar cases + outcomes"]
    S3B["📡 3B: RAG Manual Fetch<br/>Vertex AI Vector Search: OEM manuals,<br/>repair guides, parts catalog<br/>Returns: top-k chunks + source refs + confidence"]
    S3C["🛡️ 3C: SOPs & Safety Fetch<br/>Firestore: LOTO procedures, IEC 62443,<br/>NFPA 70E, escalation limits, skill tier map<br/>Returns: required safety steps, LOTO Y/N"]

    S3A --> S4
    S3B --> S4
    S3C --> S4

    subgraph S4["4️⃣ Analyser, Validator & Standards Enforcer"]
        VA1["Conflict resolution: SOPs &gt; History &gt; Manual"]
        VA2["Validation (all blocking):<br/>LOTO completeness · skill-tier confirmed<br/>IEC 62443/NFPA 70E · re-failure rate flag<br/>Auto-escalation: &gt;480V or confined space"]
        VA3["📦 ValidatedDiagnosticContext {procedure_steps[],<br/>safety_steps[], parts_list[], estimated_time,<br/>historical_success_rate, sources[], warnings[]}"]
        VA1 --> VA2 --> VA3
    end

    S4 -->|only if validation passed| S5

    subgraph S5["5️⃣ Synthesizer (Gemini 1.5 Pro)"]
        G1["Step-by-step procedure (mobile-optimized)<br/>Safety warnings + regulatory citations<br/>Parts list + SAP inventory status<br/>Time estimate · sources · bilingual output"]
        G2["Side effects: create SAP PM work order ·<br/>log to BigQuery · trigger parts pre-order"]
        G1 --> G2
    end

    S5 --> OUT["✅ Technician receives diagnostic guide<br/>(online: real-time stream / offline: cached)"]

    style S1 fill:#e8f4fd,stroke:#4a9eca
    style S2 fill:#fff3cd,stroke:#f0a500
