...
```mermaid
flowchart TD
    subgraph APP["📱 Mobile App (Android / iOS) - Flutter, MDM deployed"]
        IC["🎙️ Input Capture<br/>Voice (primary, on-device + cloud STT)<br/>Photo capture · QR/barcode scan<br/>Text (glove-friendly fallback)"]
        OFF["📴 Offline Mode<br/>Firebase local cache: pre-generated guides<br/>top 50 failures × 300 machine models<br/>Firestore offline persistence"]
        SESS["📋 Session State<br/>machine_id · active repair session<br/>step progress · escalation state"]
    end

    APP -->|"HTTPS/REST + Firebase Realtime"| GW["🚪 API Gateway + Backend (Cloud Run)<br/>Firebase Auth + IAP (identity + skill tier)<br/>Rate limit: 300 concurrent sessions<br/>Session context: Firestore"]

    subgraph IPP["⚙️ Input Processing Pipeline (Cloud Run)"]
        V2T["🗣️ Voice → Text<br/>Vertex AI Speech-to-Text (noise-suppress)"]
        IMG["🖼️ Image Analysis<br/>Gemini 1.5 Pro Vision:<br/>component, failure type, severity, part match"]
        MID["🔎 Machine ID Resolution<br/>QR scan → Firestore machine registry<br/>→ model, location, fault codes, maintenance date"]
        UPD["📦 Unified Problem Description<br/>{machine_id, model, location, fault_codes[],<br/>symptom_text, image_analysis, technician_id, skill_tier}"]
        V2T --> UPD
        IMG --> UPD
        MID --> UPD
    end
    ...
    subgraph DA["🧠 Diagnostic Agent (Cloud Run - ADK / LangGraph)"]
        S1["1️⃣ Failure Mode Classification<br/>Gemini 1.5 Pro → failure_mode + component<br/>confidence &gt;0.85 → proceed; low → ask clarifying Q"]
        S2["2️⃣ Hybrid Knowledge Retrieval<br/>graph_query → Spanner Graph (procedure, parts, skill level)<br/>vector_search → manuals + historical outcomes<br/>get_machine_context → Firestore"]
        S3["3️⃣ Safety Gate (HARD STOP)<br/>Skill-level gate · LOTO requirement (mandatory prepend)<br/>Escalation triggers (structural damage, value, time)"]
        S4["4️⃣ Instruction Generation<br/>Gemini 1.5 Pro synthesizes procedure from<br/>graph template + manuals + context + history"]
        S5["5️⃣ Real-time Integrations (parallel, non-blocking)<br/>check_parts_inventory · create_work_order<br/>check_maintenance_history"]
        S1 --> S2 --> S3 --> S4 --> S5
    end
    IPP --> DA

    subgraph KB["🗄️ Knowledge Base Layer"]
        SG[("🕸️ Spanner Graph<br/>Machine → Component → Failure → Procedure")]
        VS[("📡 Vertex AI Vector Search<br/>repair manuals, historical logs,<br/>safety bulletins, parts catalog")]
        FS[("🔥 Firestore<br/>machine registry, technician profiles,<br/>active sessions, offline sync queue")]
    end
    DA --> KB

    KB --> FL["🔁 Feedback + Learning Loop<br/>Technician marks outcome → Pub/Sub → BigQuery<br/>Weekly review by maintenance engineers<br/>Monthly RAGAS-style eval"]

    style APP fill:#e8f4fd,stroke:#4a9eca
    style IPP fill:#fff3cd,stroke:#f0a500
    style DA fill:#d4edda,stroke:#28a745
    style KB fill:#e2d9f3,stroke:#6f42c1
    style FL fill:#fde8d8,stroke:#fd7e14
```
...
```mermaid
flowchart TD
    A["📄 Repair manuals (PDFs)"] --> B["⚙️ Document AI<br/>OCR + layout detection"]
    B --> C["✂️ Chunker<br/>Semantic chunks: 512 tokens, procedure-boundary aware<br/>Metadata: machine_model, section_type, procedure_id<br/>Image extraction → Cloud Storage, linked to chunk"]
    C --> D["🧬 text-embedding-005"] --> E[("📡 Vertex AI Vector Search<br/>RAG index")]

    F["🗃️ Structured CMMS data<br/>(SAP PM export)"] --> G["⚙️ Dataflow: transform"]
    G --> H[("🕸️ Spanner Graph upsert<br/>Nodes: Machine, Model, Component,<br/>FailureMode, RepairProcedure, Part<br/>Edges: HAS_COMPONENT, EXHIBITS_FAILURE,<br/>RESOLVED_BY, REQUIRES_PART")]

    I["📊 Historical repair logs<br/>(CMMS export)"] --> J["⚙️ Outcome annotator<br/>Parse: machine_id, failure_description,<br/>steps_taken, outcome, duration<br/>Link to procedure_id in knowledge graph"]
    J --> E
```
