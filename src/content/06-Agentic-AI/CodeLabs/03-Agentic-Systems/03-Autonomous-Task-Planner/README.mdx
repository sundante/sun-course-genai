...
```mermaid
flowchart TD
    G["🎯 Goal Input"] --> P["🧭 Planner Agent"] --> TL["📋 Task Ledger<br/>(list of subtasks)"]
    TL --> E["⚙️ Executor<br/>Execute next ready subtask"]
    E --> M["🔍 Monitor Agent<br/>Check result quality"]
    M --> Q{"Result OK?"}
    Q -->|YES| N["✅ Mark complete → next subtask"]
    Q -->|NO| RP["🔁 Replanner<br/>Update remaining plan → retry"]
    N -.->|more subtasks| E
    RP -.-> E
    N -->|all subtasks done| S["🧩 Synthesizer"] --> F(["✅ Final output"])
