...
```mermaid
flowchart TD
    D["📄 Code Diff Input"] --> O["🧭 Orchestrator"]
    O --> A["🔍 Static Analysis Agent"]
    O --> B["🛡️ Security Review Agent"]
    O --> C["🎨 Style Check Agent"]
    O --> E["🧮 Complexity Agent"]
    A --> AG["🧩 Aggregator Agent"]
    B --> AG
    C --> AG
    E --> AG
    AG --> RD["📝 Review Draft"] --> CR["🧐 Reflexion: Critic"] --> F(["✅ Final Report"])
