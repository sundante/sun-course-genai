...
```mermaid
flowchart TD
    O["🧭 Orchestrator<br/>(has the plan)"] --> A1["🤖 Agent_1: task_A"]
    O --> A2["🤖 Agent_2: task_B (parallel)"]
    A1 --> W["⏳ Waits for both results"]
    A2 --> W
    W --> A3["🤖 Agent_3: task_C<br/>(using results from A and B)"]
    A3 --> S(["✅ Synthesizes final output"])
```
...
```mermaid
flowchart LR
    S[("🗂️ Shared State<br/>todo: [A, B, C], done: [], results: {}")]
    A1["🤖 Agent_1<br/>claims task_A → executes → writes result"] <--> S
    A2["🤖 Agent_2<br/>claims task_B → executes → writes result"] <--> S
    A3["🤖 Agent_3<br/>waits for A and B → executes task_C → writes result"] <--> S
```
...
```mermaid
flowchart LR
    R["🔎 Research Agent"] -->|publishes| E1["📣 ResearchCompleted<br/>results=[...]"]
    E1 -->|triggers| W["✍️ Writing Agent"]
    W -->|publishes| E2["📣 DraftCompleted"]
    E2 -->|triggers| C["🧐 Critic Agent"]
```
...
```mermaid
flowchart LR
    O["🧭 Orchestrator Agent<br/>(single agent, all tools attached)"] --> T["🔧 All tools"]
    T <-->|read/write| CS[("🗂️ CentralState")]
```
...
```mermaid
flowchart LR
    A["🤖 order_agent"] --> T1["🔧 tools"] --> GS[("🗂️ GlobalOrderState<br/>single source of truth")]
    B["🤖 inventory_agent"] --> T2["🔧 tools"] --> GS
    C["🤖 payment_agent"] --> T3["🔧 tools"] --> GS
```
...
sequenceDiagram
    ...
    participant RS as 🗂️ ReactiveOrderState
    participant EB as 📣 EventBus
    participant H as 🔔 Subscribers

    OA->>RS: state.create_order()
    RS->>EB: publishes OrderEvent.CREATED
    EB->>H: dispatches to subscribers
    H->>H: _on_order_created()<br/>appends to audit log,<br/>triggers next-stage notification
```
