...
```mermaid
flowchart LR
    LLM["🧠 LLM"] <--> TE["🔧 Tool Executor"] <--> LC["🔁 Loop Controller"]
```
...
```mermaid
xychart-beta
    title "Context size over time"
    x-axis "Steps" [1, 5, 10, 15, 20, 25, 30]
    y-axis "Tokens" 0 --> 55000
    bar [3000, 12000, 22000, 30000, 38000, 45000, 50000]
```
...
Sequential (3 API calls, 3× the latency) vs. Parallel (1 API call, all at once):
```mermaid
flowchart TD
    subgraph SEQ["🐢 Sequential - 3 API calls, 3× latency"]
        S1["1️⃣ search Tokyo weather"] --> S2["2️⃣ search Paris weather"] --> S3["3️⃣ search London weather"]
    end
    subgraph PAR["⚡ Parallel - 1 API call, all at once"]
        P0["Single API call"] --> P1["search Tokyo weather"]
        P0 --> P2["search Paris weather"]
        P0 --> P3["search London weather"]
        P1 & P2 & P3 --> R(["✅ All three results at once"])
    end
...
sequenceDiagram
    ...
    loop repeats indefinitely
        A->>T: search("answer to question")
        T-->>A: "No results found"
    end
```
