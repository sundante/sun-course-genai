...
```mermaid
flowchart LR
    G["🤖 Generator Agent"] --> D["📝 Draft"] --> E["🧪 Evaluator Agent"] --> C["💬 Critique"] --> G2["🤖 Generator Agent"] --> R(["✅ Revision"])
```
...
```mermaid
flowchart TD
    I["📥 Incoming request"] --> R{"🧭 Router Agent<br/>classify intent"}
    R -->|"billing question"| B["💳 Billing Agent"]
    R -->|"technical support"| T["🔧 Tech Support Agent"]
    R -->|"general inquiry"| G["💬 General Agent"]
    R -->|"escalation"| H["🧑 Human Support Queue"]
...
```mermaid
flowchart TD
    A["🤖 Agent plans to:<br/>Send invoice to client@company.com for $15,000"] --> G["🚧 HITL Gate triggered<br/>(irreversible action + &gt;$10,000 threshold)"]
    G --> P["👀 Present to human:<br/>action preview + justification + approve/reject"]
    P -->|Approve| S(["✅ Agent sends invoice"])
    P -->|Reject| AB(["❌ Agent aborts and reports reason"])
    P -->|Timeout| ES(["⏫ Agent escalates to supervisor"])
