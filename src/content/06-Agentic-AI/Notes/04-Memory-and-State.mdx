...
```mermaid
flowchart TD
    subgraph SESSION["⏱️ TIMESCALE: task session"]
        M1["1️⃣ In-Context Memory (short-term)<br/>The LLM's active context window.<br/>Limited. Lost when session ends."]
        M2["2️⃣ Working Memory (task state)<br/>Structured state outside the context window.<br/>Task ledger, partial results, assignments.<br/>Persisted to a database. Survives restarts."]
    end
    subgraph PERSIST["♾️ TIMESCALE: cross-task, persistent"]
        M3["3️⃣ Episodic Memory (event log)<br/>Sequential log of everything that happened.<br/>Append-only. Debugging, auditing, training data."]
        M4["4️⃣ Semantic Memory (long-term knowledge)<br/>Vector store of accumulated knowledge<br/>and preferences. Queried semantically."]
    end

    style SESSION fill:#e8f4fd,stroke:#4a9eca
...
```mermaid
flowchart TD
    subgraph START["🚀 Task Start"]
        S1["Query semantic memory for relevant knowledge"]
        S2["Load working memory if resuming"]
        S3["Initialize in-context memory<br/>(system prompt + task ledger)"]
        S4["Start appending to episodic memory"]
    end
    subgraph STEP["🔁 Each Step"]
        T1["Read from in-context memory"]
        T2["Read/write working memory"]
        T3["Append to episodic memory"]
        T4["Optionally query semantic memory"]
    end
    subgraph END_["🏁 Task End"]
        E1["Extract findings → write to semantic memory"]
        E2["Finalize working memory"]
        E3["Archive episodic memory trace"]
        E4["Clear in-context memory"]
    end

    START --> STEP --> END_

    style START fill:#e8f4fd,stroke:#4a9eca
    style STEP fill:#fff3cd,stroke:#f0a500
    style END_ fill:#d4edda,stroke:#28a745
```
