...
```mermaid
flowchart TD
    A["🚪 Interface Layer<br/>API gateway, UI, webhook handlers, auth"]
    B["🧭 Orchestration Layer<br/>Workflow engine, task graph, HITL gates, routing"]
    C["🤖 Agent Pool<br/>Specialized agents, each with their own<br/>prompt + tool set + model selection"]
    D["🔧 Tool Layer<br/>APIs, search engines, code execution, databases,<br/>file systems - each sandboxed and rate-limited"]
    E["🧠 Memory Layer<br/>Short-term (context), working (state),<br/>long-term (vector store), episodic (event log)"]
    F["📊 Monitoring Layer<br/>Tracing, logging, alerting, cost tracking"]
    A --> B --> C --> D --> E --> F
    ...
    style F fill:#fde8d8,stroke:#fd7e14
```
...
```mermaid
flowchart TD
    A["🆕 New task arrives"] --> B["🔍 Query long-term memory:<br/>What do we already know about this topic?"]
    B --> C["📂 Load working memory:<br/>What's the current state of this task?"]
    C --> D["⚙️ Agent executes<br/>(uses short-term memory = context window)"]
    D --> E["💾 Write results to<br/>working memory (state update)"]
    E --> F["📝 Append to episodic memory<br/>(event log)"]
    F -.->|periodically| G["📚 Extract important findings →<br/>write to long-term memory"]
```
...
```mermaid
flowchart LR
    C["🧑 Client submits task"] --> Q["📬 Task Queue"] --> W["⚙️ Worker picks up task"]
    W --> A["🤖 Agent runs"] --> R[("💾 Result stored")] --> H["🔔 Webhook fired"]
```
...
```mermaid
flowchart LR
    Q["📬 Task Queue"] --> W1["⚙️ Worker 1"]
    Q --> W2["⚙️ Worker 2"]
    Q --> W3["⚙️ Worker 3"]
    Q -.->|scale by adding workers| WN["⚙️ Worker N..."]
```
