...
```mermaid
flowchart LR
    A["1️⃣ DEFINE<br/>Describe tools in the API request"] --> B["2️⃣ DECIDE<br/>LLM chooses which tool to call"]
    B --> C["3️⃣ EXECUTE<br/>Framework runs the function"]
    C --> D["4️⃣ RETURN<br/>Result fed back to LLM"]
    D -.->|LLM continues reasoning| B
```
