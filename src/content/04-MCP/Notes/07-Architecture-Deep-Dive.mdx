...
← **Back to Overview:** [MCP](../INDEX.mdx)
...
```mermaid
flowchart TD
    subgraph H["🖥️ MCP HOST"]
        subgraph C1["🔌 MCP Client 1"]
            D1["📦 DATA LAYER<br/>JSON-RPC 2.0"]
            T1["🚚 TRANSPORT<br/>stdio or HTTP"]
            D1 --> T1
        end
        subgraph C2["🔌 MCP Client 2 ..."]
            D2["📦 DATA LAYER<br/>JSON-RPC 2.0"]
            T2["🚚 TRANSPORT<br/>stdio or HTTP"]
            D2 --> T2
        end
    end
    T1 --> SA["🗄️ MCP Server A<br/>(local, stdio)"]
    T2 --> SB["🗄️ MCP Server B<br/>(remote, HTTP/SSE)"]

    style H fill:#f4f1ec,stroke:#ccc4b8
```
...
sequenceDiagram
    ...
    Host->>Server: starts server process (e.g., python github_server.py)
    Host->>Server: writes initialize request to stdin
    Server->>Host: writes initialize response to stdout
    Host->>Server: writes initialized notification to stdin
...
```mermaid
flowchart TD
    S1["1️⃣ Transport Established<br/>Stdio: subprocess started<br/>HTTP: TCP connection established"]
    S2["2️⃣ Initialization<br/>initialize → initialize response → initialized notification<br/>Session is now ACTIVE"]
    S3["3️⃣ Discovery<br/>tools/list, resources/list, prompts/list"]
    S4["4️⃣ Operation (repeats indefinitely)<br/>tool calls, resource reads, prompt gets<br/>results, notifications, sampling/elicitation"]
    S5["5️⃣ Shutdown<br/>Either side closes transport<br/>Server cleans up per-session state<br/>In-flight requests abandoned"]
