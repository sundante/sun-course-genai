...
```mermaid
flowchart TD
    A["📦 DATA LAYER<br/>JSON-RPC 2.0 message framing<br/>Lifecycle management (init/shutdown)<br/>Primitives: Tools, Resources, Prompts<br/>Capabilities: Sampling, Elicitation<br/>Utilities: Progress, Cancellation, Logging"]
    B["🚚 TRANSPORT LAYER<br/>How bytes get from A to B<br/>Stdio (local subprocess)<br/>Streamable HTTP (remote server)<br/>Custom transports (implementable)"]
    A --> B
...
sequenceDiagram
    ...
    C->>S: stdin
    S-->>C: stdout
    S-->>C: stderr (server logs, optional)
```
...
sequenceDiagram
    ...
    C->>S: HTTP POST
    S-->>C: HTTP response (single response or SSE stream)
    C->>S: HTTP GET (open SSE stream)
