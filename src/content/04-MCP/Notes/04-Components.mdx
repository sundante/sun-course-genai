...
```mermaid
flowchart TD
    subgraph H["🖥️ MCP Host<br/>(e.g., Claude Desktop, VS Code)"]
        C1["🔌 Client 1"]
        C2["🔌 Client 2 ..."]
    end
    C1 --> SA["🗄️ Server A<br/>(GitHub)"]
    C2 --> SB["🗄️ Server B<br/>(Postgres)"]
...
stateDiagram-v2
    [*] --> CONNECTING
    CONNECTING --> INITIALIZING: initialize request sent
    INITIALIZING --> ACTIVE: initialize response received +\ninitialized notification sent
    ACTIVE --> ACTIVE: tool calls, resource reads,\nnotifications, etc.
    ACTIVE --> TERMINATED: either side closes transport
    TERMINATED --> CONNECTING: reconnect if desired
    TERMINATED --> [*]
```
