...
```mermaid
flowchart LR
    subgraph BEFORE["❌ BEFORE MCP - M×N = 4 connectors"]
        A1["App A"] -->|custom| S1["Source 1"]
        A1 -->|custom| S2["Source 2"]
        A2["App B"] -->|custom| S1
        A2 -->|custom| S2
    end
    subgraph AFTER["✅ WITH MCP - M+N = 4 implementations, unlimited connections"]
        B1["App A"] -->|MCP client| MS1["Source 1<br/>(MCP server)"]
        B2["App B"] -->|MCP client| MS1
        B1 -->|MCP client| MS2["Source 2<br/>(MCP server)"]
        B2 -->|MCP client| MS2
    end
...
```mermaid
flowchart TD
    A["🧑 User consent<br/>which servers can the Host connect to?"]
    B["📦 App consent<br/>which capabilities does the Host declare?"]
    C["❓ Elicitation<br/>does the user approve this specific server action?"]
    D["🔧 Tool call<br/>does the user see and approve this tool invocation?"]
    A --> B --> C --> D
```
...
```mermaid
flowchart TD
    A["🧑 User<br/>highest authority"] --> B["🖥️ Host<br/>mediates all server interactions"]
    B --> C["🔌 Client<br/>executes protocol mechanics"]
    C --> D["⚙️ Server<br/>untrusted third-party code"]

    style A fill:#d4edda,stroke:#28a745
    style D fill:#f8d7da,stroke:#dc3545
```
*Most trusted (top) → least trusted (bottom).*
