...
```mermaid
flowchart LR
    A["🖥️ Claude Desktop"] --> G1["🔌 GitHub connector<br/>(custom)"]
    A --> S1["🔌 Slack connector<br/>(custom)"]
    A --> P1["🔌 Postgres connector<br/>(custom)"]
    B["💻 VS Code AI"] --> G2["🔌 GitHub connector<br/>(custom, different)"]
    B --> S2["🔌 Slack connector<br/>(custom, different)"]
    D["..."] -.-> D
```
