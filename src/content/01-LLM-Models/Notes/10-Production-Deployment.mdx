...
> For hands-on depth on this stack - Paged Attention internals, GPTQ/AWQ serving-time quantization, batching/concurrency capacity planning, streaming and TTFT, and a real FastAPI + vLLM code lab with a load test - see [08-Serving-and-Inference](../../08-Serving-and-Inference/INDEX.mdx). This file stays at the conceptual overview level; that module doesn't re-derive what's here.
...
```mermaid
sequenceDiagram
    participant St as Static Batch Slots
    participant Ct as Continuous Batch Slots

    rect rgb(248, 215, 218)
    St->>St: Steps 1-3 [A B C D] - A, B finish
    St->>St: Steps 4-10 [_ _ C D] - 2 empty slots wasted
    end

    rect rgb(212, 237, 218)
    Ct->>Ct: Steps 1-3 [A B C D] - A, B finish
    Ct->>Ct: Step 4 [E F C D] - E, F fill immediately
    Ct->>Ct: Steps 5-8 [E F C D] - full batch throughout
    end
```
