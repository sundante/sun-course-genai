...
```mermaid
flowchart LR
    A["🏗️ Base model"] --> B["1️⃣ SFT on instruction dataset"] --> C["🎯 SFT model"]
    C --> D["2️⃣ Reward Model Training<br/>Human preference: A vs B → prefer A<br/>(prompt, response) → scalar reward"]
    D --> E["🏆 Reward model"]
    C --> F["3️⃣ PPO Reinforcement Learning<br/>Maximize reward, KL-constrained to SFT"]
    E --> F
    F --> G["✅ Aligned policy model"]

    style B fill:#e8f4fd,stroke:#4a9eca
...
Want to actually run a LoRA/QLoRA fine-tune, not just the math? See the [Fine-Tuning Lab](../../07-Fine-Tuning-Lab/INDEX.mdx) module - hands-on `peft`/`bitsandbytes` configuration, instruction data formatting and loss masking, a runnable QLoRA training script, and a measured base-vs-tuned benchmark (quality, latency, VRAM, cost).
