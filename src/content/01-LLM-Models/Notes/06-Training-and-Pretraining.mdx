...
```mermaid
flowchart TD
    A["🌐 Raw web text"] --> B["🚫 URL/domain filtering<br/>block adult content, spam, low-quality domains"]
    B --> C["🌍 Language detection<br/>keep target languages"]
    C --> D["🔁 Exact deduplication<br/>MinHash, exact hash"]
    D --> E["🔁 Near-deduplication<br/>SimHash, n-gram overlap"]
    E --> F["⭐ Quality filtering<br/>perplexity · heuristic rules · classifier scoring"]
    F --> G["🔒 PII removal<br/>email, phone numbers, SSNs"]
    G --> H["✅ Final tokenization + storage"]
```
