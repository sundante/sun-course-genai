...
flowchart TD
    subgraph G0["🖥️ GPU 0"]
        A0["Model copy A<br/>Batch 1 → grad_1"]
    end
    subgraph G1["🖥️ GPU 1"]
        A1["Model copy A<br/>Batch 2 → grad_2"]
    end
    subgraph G2["🖥️ GPU 2"]
        A2["Model copy A<br/>Batch 3 → grad_3"]
    end
    A0 --> AR["🔄 All-reduce<br/>grad = (grad_1 + grad_2 + grad_3) / 3"]
    A1 --> AR
    A2 --> AR
    AR --> U["✅ Update: W = W - lr × grad<br/>(on every GPU)"]
```
...
```mermaid
flowchart LR
    IN["📥 Linear(d_model, d_ffn)"] --> G0["🖥️ GPU 0<br/>first d_ffn/2 columns → output_0"]
    IN --> G1["🖥️ GPU 1<br/>last d_ffn/2 columns → output_1"]
    G0 --> AR["🔄 All-reduce<br/>output = concat(output_0, output_1)"]
    G1 --> AR
```
...
```mermaid
sequenceDiagram
    participant G0 as 🖥️ GPU 0 (Layers 1-16)
    participant G1 as 🖥️ GPU 1 (Layers 17-32)

    G0->>G0: process micro-batch 1
    G0->>G1: send activations (micro-batch 1)
    par
        G0->>G0: process micro-batch 2
    and
        G1->>G1: process micro-batch 1
    end
    G0->>G1: send activations (micro-batch 2)
    Note over G0,G1: ... continues until all micro-batches complete
```
...
Want to apply this VRAM/quantization math to an actual training run? See the [Fine-Tuning Lab](../../07-Fine-Tuning-Lab/INDEX.mdx) module - configuring `BitsAndBytesConfig` for real 4-bit QLoRA loading, and a benchmark script that reads *measured* peak VRAM (`torch.cuda.max_memory_allocated()`) rather than estimating it.
